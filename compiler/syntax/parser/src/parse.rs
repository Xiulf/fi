...
impl<'a, D> ParseBuffer<'a, D> {
    pub fn new(cursor: Cursor<'a>, data: D, start: Span) -> ParseBuffer<'a, D> {
        ParseBuffer {
            data,
    ...
    pub fn error<T, I: Into<String>, C: Into<String>>(&self, msg: I, code: C) -> Result<T> {
        Err(Diagnostic::error()
            .with_message(msg)
            .with_code(code)
            .with_labels(vec![Label::primary(self.cursor().file, self.span())]))
    }

    pub fn error_at<T, I: Into<String>, C: Into<String>>(
        &self,
        msg: I,
        code: C,
        span: Span,
        ...
        Err(Diagnostic::error()
            .with_message(msg)
            .with_code(code)
            .with_labels(vec![Label::primary(self.cursor().file, span)]))
    }
...
impl<'c, 'a> StepCursor<'c, 'a> {
    pub fn error<T: Into<String>>(self, message: T) -> Diagnostic<FileId> {
        Diagnostic::error()
